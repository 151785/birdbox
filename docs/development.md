...
- 资源迁移必须遵循“显式资源优先、旧字段只读兼容”原则：升级或读取旧库存时，禁止根据节点级历史字段自动创建、覆盖、启用或下发 Direct/Kernel 等资源；只有库存中已经存在的资源，或用户在资源管理中明确创建的资源，才允许参与生成配置。任何自动生成资源的行为都必须先有迁移设计、开关、回滚方案和历史库存回归测试。
- 远端节点操作必须隔离失败影响：单节点预检、应用、状态查询或超时不得阻塞其它节点和控制器 HTTP 响应；健康汇总使用有界超时和逐节点错误降级。涉及单节点的操作只锁定该节点（跨节点变更才获取联合锁），不得用单个节点故障锁死整个控制器。
