...
func TestLoadConfigRequiresAllValues(t *testing.T) {
	...
	if _, err := loadConfig(); err == nil { t.Fatal("expected missing config error") }
}
...
func TestExecuteTaskRejectsUnsupportedMethod(t *testing.T) {
	...
	if result.OK || !strings.Contains(result.Stderr, "unsupported") { t.Fatalf("unexpected result: %#v", result) }
}
...
func TestExecuteTaskRejectsNulCommand(t *testing.T) {
	...
	if result.OK || result.Code != "INVALID_COMMAND" { t.Fatalf("unexpected result: %#v", result) }
}
